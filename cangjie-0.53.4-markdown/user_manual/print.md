...
目前仓颉工具链已适配部分版本的 Linux 和 Windows 平台，但是仅针对部分 Linux 发行版做了完整功能测试，详情可参阅附录[Linux 版本工具链的支持与安装](source_zh_cn/first_understanding/../../Appendix/linux_toolchain_install.html)章节，在暂未进行过完整功能测试的平台上，仓颉工具链的功能完整性不受到保证。此外，当前 Windows 平台上的仓颉编译器基于 MinGW 实现，相较于 Linux 版本的仓颉编译器，功能会有部分欠缺，二者的具体差异请见版本 Release Note。
...
此外，仓颉工具链还依赖 OpenSSL 3 组件，由于该组件可能无法从以上发行版的默认软件源直接安装，因此你需要自行手动安装，安装方式请参考附录[Linux 版本工具链的支持与安装](source_zh_cn/first_understanding/../../Appendix/linux_toolchain_install.html)章节。


**其他 Linux 发行版**


您可以在附录[Linux 版本工具链的支持与安装](source_zh_cn/first_understanding/../../Appendix/linux_toolchain_install.html)章节找到更多 Linux 发行版的依赖安装命令。
...
* `Cangjie-x.y.z-linux_x64.tar.gz`：适用于 x86\_64 架构 Linux 系统的仓颉工具链
...
* 由 XID\_Start 字符开头，后接任意长度的 XID\_Continue 字符
* 由一个`_`开头，后接至少一个 XID\_Continue 字符


其中，XID\_Start、XID\_Continue 定义见[Unicode 标准](https://www.unicode.org/reports/tr31/tr31-37.html)。仓颉使用 Unicode 标准 15\.0\.0。
...
在仓颉编程语言中，条件表达式分为 `if` 表达式和 `if-let` 表达式两种，它们的值与类型需要根据使用场景来确定。循环表达式有四种：`for-in` 表达式、`while` 表达式、`do-while` 表达式和 `while-let` 表达式，它们的类型都是 `Unit`、值为 `()`。其中 `if-let` 表达式和 `while-let` 表达式都与模式匹配相关，请参见[if\-let 表达式](source_zh_cn/basic_programming_concepts/../enum_and_pattern_match/if_let.html)和[while\-let 表达式](source_zh_cn/basic_programming_concepts/../enum_and_pattern_match/while_let.html)章节，本节只介绍以上提及的其他几种表达式。
...
* 当含 `else` 分支的 `if` 表达式被求值时，需要根据求值上下文确定 `if` 表达式的类型：
	...
	+ 如果上下文没有明确的类型要求，则 `if` 表达式的类型是其各分支代码块类型的最小公共父类型，如果最小公共父类型不存在，编译会报错。如果编译通过，则 `if` 表达式的值就是所执行分支代码块的值。
...
关于区间类型的详细内容，请参阅基本数据类型[区间类型](source_zh_cn/basic_programming_concepts/../basic_data_type/range.html)章节。
...
[break 与 continue 表达式](#break-与-continue-表达式)
---------------------------------------------
...
详细介绍可参考[函数](source_zh_cn/basic_programming_concepts/../function/define_functions.html)模块介绍。
...
程序具体使用哪种整数类型，取决于该程序中需要处理的整数的性质和范围。在 `Int64` 类型适合的情况下，首选 `Int64` 类型，因为 `Int64` 的表示范围足够大，并且[整数类型字面量](source_zh_cn/basic_data_type/./integer.html#%E6%95%B4%E6%95%B0%E7%B1%BB%E5%9E%8B%E5%AD%97%E9%9D%A2%E9%87%8F)在没有类型上下文的情况下默认推断为 `Int64` 类型，可以避免不必要的类型转换。
...
整数类型默认支持的操作符包括：算术操作符、位操作符、关系操作符、自增和自减操作符、赋值操作符、复合赋值操作符。各操作符的优先级参见附录中的[操作符](source_zh_cn/basic_data_type/../Appendix/operator.html)。
...
1. 算术操作符包括：一元负号（`-`）、加法（`+`）、减法（`-`）、乘法（`*`）、除法（`/`）、取模（`%`）、幂运算（`**`）。
	...
	* `**` 的左操作数只能为 `Int64` 类型或 `Float64` 类型，并且：
		...
		+ 当左操作数类型为 `Float64` 时，右操作数只能为 `Int64` 类型或 `Float64` 类型，表达式的类型为 `Float64`。幂运算的使用，见如下示例：
...
> 
> 
> 本章中我们所提及的某个类型支持的操作，均是指在没有[操作符重载](source_zh_cn/basic_data_type/../function/operator_overloading.html)的前提下。
...
**多行字符串字面量**开头结尾需各存在三个双引号（`"""`）或三个单引号（`'''`）。字面量的内容从开头的三个引号换行后的第一行开始，到结尾的三个引号之前为止，之间的内容可以是任意数量的（除单独出现的 `\` 之外的）任意字符。不同于单行字符串字面量，多行字符串字面量可以跨越多行。举例如下：
...
区间类型用于表示拥有固定步长的序列，区间类型是一个泛型（详见[泛型](source_zh_cn/basic_data_type/../generic/generic_overview.html)章节），使用 `Range<T>` 表示。当 `T` 被实例化不同的类型时（要求此类型必须支持关系操作符，并且可以和 `Int64` 类型的值做加法），会得到不同的区间类型，如最常用的 `Range<Int64>` 用于表示整数区间。
...
非命名参数和命名参数的主要差异在于调用时的不同，具体可参见下文[调用函数](source_zh_cn/function/./call_functions.html)中的介绍。
...
当显式地定义了函数返回值类型时，就要求函数体的类型（关于如何确定函数体的类型可参见下节[函数体](source_zh_cn/function/./define_functions.html#%E5%87%BD%E6%95%B0%E4%BD%93)）、函数体中所有 `return e` 表达式中 `e` 的类型是返回值类型的子类型。例如，对于上述 `add` 函数，显式地定义了它的返回值类型为 `Int64`，如果将函数体中的 `return a + b` 修改为 `return (a, b)`，则会因为类型不匹配而报错：
...
[函数返回值类型](source_zh_cn/function/./define_functions.html#%E5%87%BD%E6%95%B0%E8%BF%94%E5%9B%9E%E5%80%BC%E7%B1%BB%E5%9E%8B)中我们提到函数体也是有类型的，函数体的类型是函数体内最后一“项”的类型：若最后一项为表达式，则函数体的类型是此表达式的类型，若最后一项为变量定义或函数声明，或函数体为空，则函数体的类型为 `Unit`。例如：
...
若一个函数在当前作用域中被重载（见[函数重载](source_zh_cn/function/./function_overloading.html)）了，那么直接使用该函数名作为表达式可能产生歧义，如果产生歧义编译器会报错，例如：
...
### [Composition 表达式](#composition-表达式)
...
* 操作符函数的参数个数需要匹配对应操作符的要求（详见附录[操作符](source_zh_cn/function/../Appendix/operator.html)）；
* 操作符函数只能定义在 class、interface、struct、enum 和 extend 中；
* 操作符函数具有实例成员函数的语义，所以禁止使用 `static` 修饰符；
* 操作符函数不能为泛型函数。


另外，需要注意的是，被重载后的操作符不改变它们固有的优先级和结合性（详见附录[操作符](source_zh_cn/function/../Appendix/operator.html)）。


[操作符重载函数定义和使用](#操作符重载函数定义和使用)
-----------------------------
...
2. 使用 `extend` 的方式为其添加操作符函数，从而实现操作符在这些类型上的重载。对于无法直接包含函数定义的类型（是指除 `struct`、`class`、`enum` 和 `interface` 之外其他的类型）或无法改变其实现的类型，比如第三方定义的 `struct`、`class`、`enum` 和 `interface`，只能采用这种方式（参见[扩展](source_zh_cn/function/../extension/extend_overview.html)）；
...
[`const` 上下文与 `const` 表达式](#const-上下文与-const-表达式)
-------------------------------------------------
...
[定义 struct 类型](#定义-struct-类型)
=============================


`struct` 类型的定义以关键字 `struct` 开头，后跟 `struct` 的名字，接着是定义在一对花括号中的 `struct` 定义体。`struct` 定义体中可以定义一系列的成员变量、成员属性（参见[属性](source_zh_cn/struct/../class_and_interface/prop.html)）、静态初始化器、构造函数和成员函数。
...
一个 `struct` 中可以定义多个普通构造函数，但它们必须构成重载（参见[函数重载](source_zh_cn/struct/../function/function_overloading.html)），否则报重定义错误。
...
[struct 成员的访问修饰符](#struct-成员的访问修饰符)
-----------------------------------


`struct` 的成员（包括成员变量、成员属性、构造函数、成员函数、操作符函数（详见[操作符重载](source_zh_cn/struct/../function/operator_overloading.html)章节））用 4 种访问修饰符修饰：`private`、`internal`、`protected` 和 `public`，缺省的含义是 `internal`。


* `private` 表示在 `struct` 定义内可见。
* `internal` 表示仅当前包及子包（包括子包的子包，详见[包](source_zh_cn/struct/../package/toplevel_access.html)章节）内可见。
* `protected` 表示当前模块（详见[包](source_zh_cn/struct/../package/toplevel_access.html)章节）可见。
* `public` 表示模块内外均可见。
...
另外，在 `enum` 体中还可以定义一系列成员函数、操作符函数（详见[操作符重载](source_zh_cn/enum_and_pattern_match/../function/operator_overloading.html)章节）和成员属性（详见[属性](source_zh_cn/enum_and_pattern_match/../class_and_interface/prop.html)章节），但是要求构造器、成员函数、成员属性之间不能重名。例如，下面的例子在 `RGBColor` 中定义了一个名为 `printType` 的函数，它会输出字符串 `RGBColor`：
...
`Option` 类型被定义为一个泛型 `enum` 类型，定义如下（这里我们仅需要知道尖括号中的 `T` 是一个类型形参，当 `T` 为不同类型时会得到不同的 `Option` 类型即可。关于泛型的详细介绍，可参见[泛型](source_zh_cn/enum_and_pattern_match/../generic/generic_overview.html)）：
...
最后，关于 `Option` 的使用，请参见[使用 Option](source_zh_cn/enum_and_pattern_match/../error_handle/use_option.html)。
...
通配符模式使用下划线 `_` 表示，可以匹配任意值。通配符模式通常作为最后一个 `case` 中的模式，用来匹配其他 `case` 未覆盖到的情况，如[常量模式](source_zh_cn/enum_and_pattern_match/./pattern_overview.html#%E5%B8%B8%E9%87%8F%E6%A8%A1%E5%BC%8F)中匹配 `score` 值的示例中，最后一个 `case` 中使用 `_` 来匹配无效的 `score` 值。
...
> 
> 
> 当模式的 identifier 为 enum 构造器时，该模式会被当成 enum 模式进行匹配，而不是绑定模式（关于 enum 模式，详见 [enum 模式](source_zh_cn/enum_and_pattern_match/pattern_overview.html#enum-%E6%A8%A1%E5%BC%8F)章节）。
...
[模式的 Refutability](#模式的-refutability)
=====================================
...
每个 `case` 分支以关键字 `case` 开头，`case` 之后是一个模式或多个由 `|` 连接的相同种类的模式（如上例中的 `1`、`0`、`_` 都是模式，详见[模式概述](source_zh_cn/enum_and_pattern_match/../enum_and_pattern_match/pattern_overview.html)章节）；模式之后可以接一个可选的 `pattern guard`，表示本条 `case` 匹配成功后额外需要满足的条件；接着是一个 `=>`，`=>` 之后即本条 `case` 分支匹配成功后需要执行的操作，可以是一系列表达式、变量和函数定义（新定义的变量或函数的作用域从其定义处开始到下一个 `case` 之前结束），如上例中的变量定义和 `print` 函数调用。
...
上例中，因为 `x` 的值等于 `0`，所以会和第二条 `case` 分支匹配（此处使用的是常量模式，匹配的是值是否相等，详见[常量模式](source_zh_cn/enum_and_pattern_match/../enum_and_pattern_match/pattern_overview.html#%E5%B8%B8%E9%87%8F%E6%A8%A1%E5%BC%8F)章节），最后输出 `x = 0`。
...
在下面的例子中（使用到了 `enum` 模式，详见 [enum 模式](source_zh_cn/enum_and_pattern_match/../enum_and_pattern_match/pattern_overview.html#enum-%E6%A8%A1%E5%BC%8F)章节），当 `RGBColor` 的构造器的参数值大于等于 `0` 时，输出它们的值，当参数值小于 `0` 时，认为它们的值等于 `0`：
...
`class` 类型的定义以关键字 `class` 开头，后跟 `class` 的名字，接着是定义在一对花括号中的 `class` 定义体。`class` 定义体中可以定义一系列的成员变量、成员属性（参见[属性](source_zh_cn/class_and_interface/prop.html)）、静态初始化器、构造函数、成员函数和操作符函数（详见[操作符重载](source_zh_cn/class_and_interface/../function/operator_overloading.html)章节））。
...
一个类中可以定义多个普通构造函数，但它们必须构成重载（参见[函数重载](source_zh_cn/class_and_interface/../function/function_overloading.html)），否则报重定义错误。
...
根据有没有函数体，实例成员函数又可以分为抽象成员函数和非抽象成员函数。抽象成员函数没有函数体，只能定义在抽象类或接口（详见[接口](source_zh_cn/class_and_interface/interface.html)章节）中。例如，下例中在抽象类 `AbRectangle`（使用关键字 `abstract` 修饰）中定义了抽象函数 `foo`。
...
* `internal` 表示仅当前包及子包（包括子包的子包，详见[包](source_zh_cn/class_and_interface/../package/toplevel_access.html)章节）内可见。
* `protected` 表示当前模块（详见[包](source_zh_cn/class_and_interface/../package/toplevel_access.html)章节）及当前类的子类可见。
* `public` 表示模块内外均可见。
...
`class` 仅支持单继承，因此下面这样一个类继承两个类的代码是不合法的（`&` 是类实现多个接口时的语法，详见[接口](source_zh_cn/class_and_interface/interface.html)章节）。
...
例如下面的 `printTypeName` 函数，当我们约束泛型变元 `T` 是 `NamedType` 的子类型时，我们需要保证 `T` 的实例化类型中所有的静态成员函数（或属性）都必须拥有实现，以保证可以使用 `T.typename` 的方式访问泛型变元的实现，达到了我们对静态成员抽象的目的。详见[泛型](source_zh_cn/class_and_interface/../generic/generic_overview.html)章节。
...
2. 通过扩展实现接口，这种方式详见[扩展](source_zh_cn/class_and_interface/../extension/interface_extension.html)章节。
...
getter 和 setter 的实现中可以和函数体一样包含声明和表达式，与函数体的规则一样，详见[函数体](source_zh_cn/class_and_interface/../function/define_functions.html#%E5%87%BD%E6%95%B0%E4%BD%93)章节。
...
属性分为实例成员属性和静态成员属性。成员属性的使用和成员变量的使用方式一样，详见[成员变量](source_zh_cn/class_and_interface/./class.html#class-%E6%88%90%E5%91%98%E5%8F%98%E9%87%8F)章节。
...
[继承 class 带来的子类型关系](#继承-class-带来的子类型关系)
---------------------------------------
...
[泛型类型的子类型关系](#泛型类型的子类型关系)
-------------------------


泛型类型间也有子类型关系，详见[泛型类型的子类型关系](source_zh_cn/class_and_interface/../generic/generic_subtype.html)章节。
...
[`Rune` 到 `UInt32` 和整数类型到 `Rune` 的转换](#rune-到-uint32-和整数类型到-rune-的转换)
---------------------------------------------------------------------
...
[泛型接口](source_zh_cn/generic/./generic_interface.html)中介绍了泛型接口的定义和使用，本节我们介绍泛型类的定义和使用。如 `Map` 的键值对就是使用泛型类来定义的。
...
由于键与值的类型有可能不相同，且可以为任意满足条件的类型，所以 `Node` 需要两个类型形参 `K` 与 `V` ，`K <: Hashable, K <: Equatable<K>` 是对于键类型的约束，意为 `K` 要实现 `Hashable` 与 `Equatable<K>` 接口，也就是 `K` 需要满足的条件。对于泛型约束，详见[泛型约束](source_zh_cn/generic/./generic_constraint.html)章节。
...
在仓颉编程语言中，泛型 enum 声明的类型里被使用得最广泛的例子之一就是 `Option` 类型了，关于 `Option` 详细描述可以详见 [Option 类型](source_zh_cn/generic/../enum_and_pattern_match/option_type.html)章节。 `Option` 类型是用来表示在某一类型上的值可能是个空的值。这样，`Option` 就可以用来表示在某种类型上计算的失败。这里是何种类型上的失败是不确定的，所以很明显，`Option` 是一个泛型类型，需要声明类型形参。
...
[基础 Collection 类型概述](#基础-collection-类型概述)
=========================================
...
[访问 ArrayList 成员](#访问-arraylist-成员)
-----------------------------------
...
ArrayList 也支持下标中使用 Range 的语法，详见 [Array](source_zh_cn/Collections/../basic_data_type/array.html#array) 章节。


[修改 ArrayList](#修改-arraylist)
-----------------------------
...
[增加 ArrayList 的大小](#增加-arraylist-的大小)
-------------------------------------
...
[Iterable 和 Collections](#iterable-和-collections)
=================================================
...
[使用 `import` 语句导入其它包中的声明或定义](#使用-import-语句导入其它包中的声明或定义)
-------------------------------------------------------
...
[使用 `import as` 对导入的名字重命名](#使用-import-as-对导入的名字重命名)
---------------------------------------------------
...
在[Option 类型](source_zh_cn/error_handle/../enum_and_pattern_match/option_type.html)中我们介绍了 Option 类型的定义，因为 Option 类型可以同时表示有值和无值两种状态，而无值在某些情况下也可以理解为一种错误，所以 Option 类型也可以用作错误处理。
...
`sleep()` 函数会让当前线程睡眠指定的时长，之后再恢复执行，其时间由指定的 Duration 类型决定，详细介绍请参考[线程睡眠指定时长](source_zh_cn/concurrency/./sleep.html)章节。
...
[使用 `Future<T>` 等待线程结束并获取返回值](#使用-futuret-等待线程结束并获取返回值)
-------------------------------------------------------
...
[可重入互斥锁 ReentrantMutex](#可重入互斥锁-reentrantmutex)
-----------------------------------------------
...
[MultiConditionMonitor](#multiconditionmonitor)
-----------------------------------------------
...
[线程局部变量 ThreadLocal](#线程局部变量-threadlocal)
-----------------------------------------
...
其中，`openRead` 打开的文件是只读的，我们不能对实例进行写操作，否则会抛出运行时异常；而 `create` 创建的文件是只写的，我们不能对实例进行读操作，否则也会抛出运行时异常。
...
因为我们打开 `File` 的实例会占用宝贵的系统资源，所以使用完 `File` 的实例之后需要注意及时关闭 `File`，以释放系统资源。


所幸 `File` 实现了 `Resource` 接口，我们在大多数时候都可以使用 try\-with\-resource 语法来简化我们的使用。
...
[Tokens 相关类型和 quote 表达式](#tokens-相关类型和-quote-表达式)
=================================================
...
* 所有的节点类型（节点将在[语法节点](source_zh_cn/Macro/./sytax_node.html)中讨论）
* `Token` 和 `Tokens` 类型
...
更多插值的用法可以参考 [使用 quote 插值语法节点](source_zh_cn/Macro/./sytax_node.html#%E4%BD%BF%E7%94%A8-quote-%E6%8F%92%E5%80%BC%E8%AF%AD%E6%B3%95%E8%8A%82%E7%82%B9)。
...
当输出这个语法树的时候，我们必须在 `a + b` 周围添加括号，得到 `(a + b) * y`（如果输出`a + b * y`，含义为先做乘法，再做加法，与语法树的含义不同）。**`ast` 库具备在输出语法树时自动添加括号的功能**。
...
[使用 quote 插值语法节点](#使用-quote-插值语法节点)
-----------------------------------
...
一般来说，插值运算符后面的表达式使用小括号限定作用域，例如 `$(binExpr)`。但是当后面只跟单个标识符的时候，小括号可省略，即可写为 `$binExpr`。因此，在案例中 `a` 和 `b` 都在 `quote` 中插入了 `binExpr`节点，结果为 `1 + 2`。然而，如果插值运算符后面的表达式更复杂，不加小括号可能造成作用域出错。例如，表达式 `binExpr.leftExpr` 求值为 `1 + 2` 的左表达式，即 `1`，因此 `c` 正确赋值为 `1`。但 `d` 中的插值被解释为 `($binExpr).leftExpr`，因此结果是 `1 + 2.leftExpr`。为了明确插值的作用域，我们推荐在插值运算符中使用小括号。
...
本章节介绍仓颉宏的定义和使用，仓颉宏可以分为[非属性宏](source_zh_cn/Macro/implementation_of_macros.html#%E9%9D%9E%E5%B1%9E%E6%80%A7%E5%AE%8F)和[属性宏](source_zh_cn/Macro/implementation_of_macros.html#%E5%B1%9E%E6%80%A7%E5%AE%8F)。同时本章节还会介绍宏出现嵌套时的行为。
...
上述代码的编译过程可以参考[宏的编译和使用](source_zh_cn/Macro/./compiling_error_reporting_and_debugging.html#%E5%AE%8F%E7%9A%84%E7%BC%96%E8%AF%91%E5%92%8C%E4%BD%BF%E7%94%A8)。
...
> 
> 
> 宏替换过程依赖仓颉 runtime ，宏替换过程中仓颉 runtime 的初始化配置采用宏提供的默认配置，配置参数支持使用仓颉 runtime 运维日志进行查询，其中 cjHeapSize 与 cjStackSize 支持用户修改，其余暂不支持，仓颉 runtime 初始化配置可参见[runtime 初始化可选配置](source_zh_cn/Macro/../Appendix/runtime_env.html#runtime%E5%88%9D%E5%A7%8B%E5%8C%96%E5%8F%AF%E9%80%89%E9%85%8D%E7%BD%AE)章节。
...
> 
> 
> 重导出的声明也允许对外可见，关于包管理和重导出的相关概念，请参见[包的导入](source_zh_cn/Macro/../package/import.html)章节。
...
编译命令如下，这里选择使用 `--output-type` 选项将 B 包编译成到动态库，关于 cjc 编译选项介绍可以参考[cjc 编译选项](source_zh_cn/Macro/../Appendix/compile_options_OHOS.html)章节。
...
关于条件编译的具体内容，可以参考[条件编译](source_zh_cn/Macro/../Compile-And-Build/conditional_compilation.html)章节，这里不再额外展开。
...
然而，这个实现需要每次对 `e` 的值进行分析，在循环和条件判断中多次对 `ve` 进行判断和更新。此外，实现只支持 `n` 的类型为`Int64`的情况，如果要支持其他类型的 `n`，还要处理如何表达 `result = 1` 的问题。如果我们预先知道 `e` 的具体值，可以将这个代码写的更简单。例如，如果知道 `e` 的值为 10，我们可以展开整个循环如下：
...
在宏的实现中，使用 while 循环从索引 0 开始依次解析每个表达式。变量 `index` 保存当前解析的位置。每次调用 `parseExprFragment` 时，从当前位置开始，并返回解析后的位置（以及解析得到的表达式）。如果解析后的位置到达了输入的结尾，则退出循环。否则检查到达的位置是一个括号，如果不是括号，报错并退出，如果是括号，跳过这个括号并开始下一轮的解析。在得到表达式的列表后，依次输出每个表达式。
...
例如我们存在一个 module0 模块下的 package0 包含一个公开的类型 Foo，其对应的仓颉动态模块路径为 "./module\_package.so" 。应用动态加载我们便可以在运行时得到这个 Foo 的类型信息。
...
[确保正确使用整数运算溢出策略的注解](#确保正确使用整数运算溢出策略的注解)
---------------------------------------
...
### [`@FastNative` 使用限制](#fastnative-使用限制)
...
2. `foreign` 声明的函数，参数和返回类型必须符合 C 和仓颉数据类型之间的映射关系（详见下节：[类型映射](source_zh_cn/FFI/./cangjie-c.html#%E7%B1%BB%E5%9E%8B%E6%98%A0%E5%B0%84)）。
...
关于仓颉编译器支持的所有编译选项，详见[cjc 编译选项](source_zh_cn/FFI/../Appendix/compile_options_OHOS.html)。
...
这里我们演示一下如何使用 C 互操作以及 `write/read` 接口对一个结构体进行赋值和读取值：
...
[Python 的全局资源及使用](#python-的全局资源及使用)
-----------------------------------


### [提供内建函数类以及全局资源](#提供内建函数类以及全局资源)
...
### [提供 Python 库日志类 `PythonLogger`](#提供-python-库日志类-pythonlogger)
...
### [提供 Python 库异常类 `PythonException`](#提供-python-库异常类-pythonexception)
...
### [提供 Python 库的版本信息类 `Version`](#提供-python-库的版本信息类-version)
...
[`PythonBuiltins` 内建函数类](#pythonbuiltins-内建函数类)
-----------------------------------------------


### [Python 库的导入和加载](#python-库的导入和加载)
...
### [Python FFI 库泛型约束的接口 `PyFFIType`](#python-ffi-库泛型约束的接口-pyffitype)
...
### [`PyBool` 与 `Bool` 的映射](#pybool-与-bool-的映射)
...
### [`PyString` 与字符、字符串的映射](#pystring-与字符字符串的映射)
...
### [`PyList` 与 `Array` 的映射](#pylist-与-array-的映射)
...
### [`PyDict` 与 `HashMap` 的映射](#pydict-与-hashmap-的映射)
...
### [`PySet` 与 `HashSet` 的映射](#pyset-与-hashset-的映射)
...
### [`PyObj` 的迭代器类型 `PyObjIterator`](#pyobj-的迭代器类型-pyobjiterator)
...
[仓颉与 Python 的注册回调](#仓颉与-python-的注册回调)
-------------------------------------
...
### [Cangjie 类型、C 类型与 Python 类型之间的映射](#cangjie-类型c-类型与-python-类型之间的映射)
...
* `PyCType` 为仓颉侧对应的 `PyCFunc` 接口配置类型，详细见[`PyCFunc` 类原型](source_zh_cn/FFI/cangjie-python.html#pycfunc-%E7%B1%BB%E5%8E%9F%E5%9E%8B)以及[示例](source_zh_cn/FFI/cangjie-python.html#%E7%A4%BA%E4%BE%8B)展示；
...
[`cjc` 基本使用方法](#cjc-基本使用方法)
---------------------------


想必你已经在学习仓颉的过程中尝试着使用 `cjc` 了，我们先来看一下 `cjc` 的基本使用方法，如果你想了解详细的编译选项内容，请查阅[“cjc 编译选项”](source_zh_cn/Compile-And-Build/../Appendix/compile_options.html)章节。
...
[`cjpm.toml` 配置文件说明](#cjpmtoml-配置文件说明)
--------------------------------------
...
### [`--output-type=[exe|staticlib|dylib]` \[frontend]](#--output-typeexestaticlibdylib-frontend)
...
### [`--package`, `-p` \[frontend]](#--package--p-frontend)
...
### [`--module-name <value>` \[frontend]](#--module-name-value-frontend)
...
### [`--output <value>`, `-o <value>`, `-o<value>` \[frontend]](#--output-value--o-value--ovalue-frontend)
...
### [`--library <value>`, `-l <value>`, `-l<value>`](#--library-value--l-value--lvalue)
...
### [`--library-path <value>`, `-L <value>`, `-L<value>`](#--library-path-value--l-value--lvalue)
...
### [`--trimpath <value>` \[frontend]](#--trimpath-value-frontend)
...
### [`--int-overflow=[throwing|wrapping|saturating]` \[frontend]](#--int-overflowthrowingwrappingsaturating-frontend)
...
### [`--diagnostic-format=[default|noColor|json]` \[frontend]](#--diagnostic-formatdefaultnocolorjson-frontend)
...
### [`--verbose`, `-V` \[frontend]](#--verbose--v-frontend)


`cjc` 会打印出编译器版本信息，工具链依赖的相关信息以及编译过程中执行的命令。


### [`--help`, `-h` \[frontend]](#--help--h-frontend)
...
### [`--version`, `-v` \[frontend]](#--version--v-frontend)
...
### [`--save-temps <value>`](#--save-temps-value)
...
### [`--import-path <value>` \[frontend]](#--import-path-value-frontend)
...
### [`--scan-dependency` \[frontend]](#--scan-dependency-frontend)
...
### [`--warn-off`, `-Woff <value>` \[frontend]](#--warn-off--woff-value-frontend)
...
### [`--warn-on`, `-Won <value>` \[frontend]](#--warn-on--won-value-frontend)
...
### [`--error-count-limit <value>` \[frontend]](#--error-count-limit-value-frontend)
...
### [`--output-dir <value>` \[frontend]](#--output-dir-value-frontend)
...
### [`--stack-trace-format=[default|simple|all]`](#--stack-trace-formatdefaultsimpleall)
...
### [`--pgo-instr-use=<.profdata>`](#--pgo-instr-useprofdata)
...
### [`--target <value>` \[frontend]](#--target-value-frontend)
...
| --- | --- |
| x86\_64\-linux\-gnu | aarch64\-hm\-gnu |
...
### [`--target-cpu <value>`](#--target-cpu-value)
...
### [`--toolchain <value>`, `-B <value>`, `-B<value>`](#--toolchain-value--b-value--bvalue)
...
### [`--strip-all`, `-s`](#--strip-all--s)


编译可执行文件或动态库时，指定该选项以删除输出文件中的符号表。


### [`--discard-eh-frame`](#--discard-eh-frame)
...
### [`--link-options <value>`1](#--link-options-value1)
...
`unittest` 测试框架提供的入口，由宏自动生成，当使用 `cjc --test` 选项编译时，程序入口不再是 `main`，而是 `test_entry`。unittest 测试框架的使用方法请参见 《仓颉语言库 API》文档。
...
### [`--mock <on|off|runtime-error>` \[frontend]](#--mock-onoffruntime-error-frontend)
...
`cjc` 支持以下宏选项，关于宏的更多内容请参阅[“宏”](source_zh_cn/Appendix/./Chapter_15_Macro.html)章节。


### [`--compile-macro` \[frontend]](#--compile-macro-frontend)


编译宏定义文件，生成默认的宏定义动态库文件。


### [`--debug-macro` \[frontend]](#--debug-macro-frontend)


生成宏展开后的仓颉代码文件。该选项可用于调试宏展开功能。


### [`--parallel-macro-expansion` \[frontend]](#--parallel-macro-expansion-frontend)
...
`cjc` 支持以下条件编译选项，关于条件编译的更多内容请参阅[“条件编译”](source_zh_cn/Appendix/../Compile-And-Build/conditional_compilation.html)。


### [`--cfg <value>` \[frontend]](#--cfg-value-frontend)
...
### [`--jobs <value>`, `-j <value>` \[frontend]](#--jobs-value--j-value-frontend)
...
### [`--aggressive-parallel-compile`, `--apc` \[frontend]](#--aggressive-parallel-compile---apc-frontend)
...
### [`--fchir-constant-propagation` \[frontend]](#--fchir-constant-propagation-frontend)


开启 chir 常量传播优化。


### [`--fno-chir-constant-propagation` \[frontend]](#--fno-chir-constant-propagation-frontend)


关闭 chir 常量传播优化。


### [`--fchir-function-inlining` \[frontend]](#--fchir-function-inlining-frontend)


开启 chir 函数内联优化。


### [`--fno-chir-function-inlining` \[frontend]](#--fno-chir-function-inlining-frontend)


关闭 chir 函数内联优化。


### [`--fchir-devirtualization` \[frontend]](#--fchir-devirtualization-frontend)


开启 chir 去虚函数调用优化。


### [`--fno-chir-devirtualization` \[frontend]](#--fno-chir-devirtualization-frontend)


关闭 chir 去虚函数调用优化。


### [`--fast-math` \[frontend]](#--fast-math-frontend)


开启此选项后，编译器会对浮点数作一些激进且有可能损失精度的假设，以便优化浮点数运算。


### [`-O<N>` \[frontend]](#-on-frontend)
...
### [`--fno-obf-string`](#--fno-obf-string)


关闭字符串混淆。


### [`--fobf-const`](#--fobf-const)
...
### [`--fno-obf-const`](#--fno-obf-const)


关闭常量混淆。


### [`--fobf-layout`](#--fobf-layout)
...
### [`--fno-obf-layout`](#--fno-obf-layout)


关闭外形混淆。


### [`--obf-sym-prefix <string>`](#--obf-sym-prefix-string)
...
### [`--obf-sym-output-mapping <file>`](#--obf-sym-output-mapping-file)
...
### [`--obf-sym-input-mapping <file,...>`](#--obf-sym-input-mapping-file)
...
### [`--obf-apply-mapping-file <file>`](#--obf-apply-mapping-file-file)
...
### [`--fobf-export-symbols`](#--fobf-export-symbols)
...
### [`--fno-obf-export-symbols`](#--fno-obf-export-symbols)


禁止外形混淆功能混淆导出符号。


### [`--fobf-source-path`](#--fobf-source-path)
...
### [`--fno-obf-source-path`](#--fno-obf-source-path)


禁止外形混淆功能混淆堆栈信息中的路径信息。


### [`--fobf-line-number`](#--fobf-line-number)
...
### [`--fno-obf-line-number`](#--fno-obf-line-number)


禁止外形混淆功能混淆堆栈信息中的行号信息。


### [`--fobf-cf-flatten`](#--fobf-cf-flatten)
...
### [`--fno-obf-cf-flatten`](#--fno-obf-cf-flatten)


关闭控制流平坦化混淆。


### [`--fobf-cf-bogus`](#--fobf-cf-bogus)
...
### [`--fno-obf-cf-bogus`](#--fno-obf-cf-bogus)
...
### [`--obf-config <file>`](#--obf-config-file)
...
### [`--obf-level <value>`](#--obf-level-value)
...
### [`--link-options "-z noexecstack"`1](#--link-options--z-noexecstack1)


设置线程栈不可执行。


### [`--link-options "-z relro"`1](#--link-options--z-relro1)


设置 GOT 表重定位只读。


### [`--link-options "-z now"`1](#--link-options--z-now1)
...
### [`--sanitizer-coverage-level=0/1/2`](#--sanitizer-coverage-level012)
...
### [`--sanitizer-coverage-trace-pc-guard`](#--sanitizer-coverage-trace-pc-guard)
...
### [`--sanitizer-coverage-inline-8bit-counters`](#--sanitizer-coverage-inline-8bit-counters)
...
### [`--sanitizer-coverage-inline-bool-flag`](#--sanitizer-coverage-inline-bool-flag)
...
### [`--sanitizer-coverage-pc-table`](#--sanitizer-coverage-pc-table)
...
### [`--sanitizer-coverage-stack-depth`](#--sanitizer-coverage-stack-depth)
...
### [`--sanitizer-coverage-trace-compares`](#--sanitizer-coverage-trace-compares)
...
### [`--sanitizer-coverage-trace-memcmp`](#--sanitizer-coverage-trace-memcmp)
...
1. 不支持仓颉宏选项；
2. 在协程 A 调用 `releaseArrayRawData` 接口后，若协程 B 仍持有该段数据的引用时，协程 A 再访问该段数据会产生**漏报**；
3. 仓颉内存释放后使用的场景中，若该片内存再次被分配且设置为可访问后，原来的野指针（产生释放后使用的指针）访问该片内存则会产生**漏报**；
4. 仓颉检测内存泄露中，若 C 内存在仓颉类中初始化，但没有在仓颉 class 的 `~init()` 中释放的场景中，会产生**漏报**；
5. 从仓颉堆内存回收到程序结束退出这一段时期程序产生的内存泄露会**漏报**。


### [`--sanitize=address`](#--sanitizeaddress)
...
2. 用户通过 `-L` ，环境变量 `LIBRARY_PATH` 或 `--toolchain/-B` 指定的路径下搜索 ASan 运行库 `clang-rt.asan-<arch>.a`；
...
1. 当用户使用 C 代码作为主程序时，还需要链接 `$CANGJIE_HOME/lib/<system>_<arch>_llvm/cjasan_options.o` 以防止内存泄露**漏报**的问题；
2. 当用户使用 ASan 的动态链接库时，需要使用 `LD_PRELOAD` 或 `verify_asan_link_order=0` 保证 ASan 运行库是第一个被加载的，以保证 ASan 的检测正确地运行；
...
`cjc` 支持数据竞争检测功能，用来检测并发的仓颉程序中存在的数据竞争缺陷。
...
### [`--sanitize=thread`](#--sanitizethread)


使能数据竞争检测功能。
开启该编译选项后，在程序运行时如果发生数据竞争，程序会打印出发生数据竞争的代码位置、调用栈和相关的协程信息。
...
\*\*值得注意的是，\*\*当用户使用 C 代码作为主程序调用仓颉库时，如果仓颉库开启了 `--sanitize=thread` 选项，则 C 代码在编译时需要添加以下参数来链接相关依赖：
...
### [`--experimental` \[frontend]](#--experimental-frontend)
